...
const sections = [
  {
    "id": "overview",
    "title": "핵심 요약",
    "body": "하드코트 경기 후 회복 가이드의 핵심은 한 가지 원칙으로 정리할 수 있습니다. 하드코트에서 누적되는 충격을 줄이기 위한 회복 루틴과 관리 포인트를 정리했습니다. 이 글은 실제 경기와 연습에서 바로 적용할 수 있는 기준을 빠르게 정리하는 데 초점을 둡니다."
  },
  {
    "id": "common-mistakes",
    "title": "자주 나오는 실수",
    "body": "많은 동호인은 상황을 단순 기술 문제로만 해석하지만, 실제로는 준비 타이밍과 선택의 문제가 더 큽니다. 하드코트 경기 후 회복 가이드를 다룰 때는 무리한 스피드보다 안정적인 반복과 명확한 의사결정이 먼저입니다."
  },
  {
    "id": "practical-drills",
    "title": "실전 적용 방법",
    "body": "연습에서는 목표를 한 번에 여러 개 두지 않는 편이 효율적입니다. 한 세션에는 한 가지 패턴만 반복하고, 다음 세션에서 속도와 정확도를 올리는 식으로 접근해야 하드코트 경기 후 회복 가이드의 효과가 실제 경기력으로 연결됩니다."
  },
  {
    "id": "checklist",
    "title": "체크리스트",
    "body": "훈련 전 목표 설정, 훈련 중 반복 수 기록, 훈련 후 피드백 정리라는 세 단계를 유지하면 개선 속도가 빨라집니다. 이 구조를 통해 하드코트 경기 후 회복 가이드 관련 훈련을 단발성 팁이 아니라 누적형 시스템으로 바꿀 수 있습니다."
  }
] as const;
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-12">
      <article className="rounded-3xl border border-black/5 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-gray-900">
        ...
        <p className="mb-8 text-base leading-7 text-gray-600 dark:text-gray-300">하드코트에서 누적되는 충격을 줄이기 위한 회복 루틴과 관리 포인트를 정리했습니다.</p>
