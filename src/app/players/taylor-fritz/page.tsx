...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: 'https://www.tennisfrens.com/players/taylor-fritz',
  },
  openGraph: {
    title: '테일러 프리츠 — 세계랭킹 상위권 | 플레이스타일·명장면·최근 경기력',
    description: '테일러 프리츠의 하드 전문 스타일, 강점, 대표 경기, 최근 흐름을 한 번에 정리한 선수 프로필.',
    url: 'https://www.tennisfrens.com/players/taylor-fritz',
...
const faqs = [
  {
    ...
    a: '하드 전문 스타일로, 직선적 포핸드와 안정된 서브가 특징입니다. 단단한 포핸드와 계산된 코스 선택으로 큰 코트의 시간을 단축합니다.',
  },
  {
  ...
  {
    q: '프리츠의 성장 배경은 어떤가요?',
    a: '미 서부의 건조하고 빠른 코트에서 자랐으며, 부모가 모두 테니스 선수였던 가정 환경에서 루틴의 생활화를 일찍 체득했습니다.',
  },
  {
    q: '프리츠를 한 문장으로 설명하면?',
    a: '직선적 포핸드와 안정된 서브로 하드코트를 설계하는 미국의 대표 선수입니다.',
  },
];
      ...
      >
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {['테니스', '테일러 프리츠', '선수 프로필', '미국', '하드 전문', '직선적 포핸드'].map((tag) => (
          ...
          <div className="prose prose-lg dark:prose-invert max-w-none">
            <h2 id="why-again-notable">테일러 프리츠, 왜 주목받는 선수인가?</h2>
            ...
            <p>테일러 프리츠는 미국 출신의 세계랭킹 상위권 테니스 선수로, 직선적 포핸드와 안정된 서브로 하드 코트를 지배하고 있다. 최고 랭킹 상위권에 올랐던 경력의 소유자이며, 특히 하드 코트에서 강력한 모습을 보여준다. 단단한 포핸드와 계산된 코스 선택이 그의 테니스를 특징짓는다.</p>
            <p>그는 인디언 웰스에서 우승을 차지하며 마스터스 대회 챔피언이 되었고, 하드 코트에서 안정적인 성적을 거두고 있다. 최근 몇 시즌 동안 하드 시즌에서 특히 강력한 모습을 보여주며, 직선의 파워와 계산된 각으로 평가받는다. 그의 테니스는 직선적 포핸드와 안정된 서브가 핵심이다.</p>

            <h2 id="what-type-of-player">이 선수는 어떤 유형의 플레이어인가?</h2>
            <p>테일러 프리츠는 미국 출신의 세계랭킹 상위권 선수로, 하드 전문 스타일과 직선적 포핸드, 안정된 서브가 특징인 ATP 투어의 안정적인 선수다.</p>
            ...
            <div className="not-prose my-6 p-5 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900/50 dark:to-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
              ...
              <ul className="space-y-3 text-sm">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <span><strong className="text-gray-700 dark:text-gray-300">시그니처 무기:</strong> <span className="text-gray-600 dark:text-gray-400">직선적 포핸드</span></span>
                </li>
              </ul>
            </div>

            <p>플레이 스타일은 하드 전문으로 분류된다. 베이스라인에서 직선적 포핸드와 안정된 서브를 구사하며, 계산된 코스 선택으로 하드코트를 설계한다. 특히 직선적 포핸드가 그의 시그니처 무기로, 단단한 포핸드와 계산된 각으로 큰 코트의 시간을 단축한다.</p>
            ...
            <p>팬들이 그를 표현할 때 자주 쓰는 말은 <strong>"직선적 포핸드"</strong>, <strong>"안정된 서브"</strong>, <strong>"미국의 대표"</strong>이다. 그의 테니스는 직선의 파워와 계산된 각을 추구하며, 특히 단단한 포핸드와 안정된 서브가 뛰어나다.</p>

            <hr className="my-8 border-gray-200 dark:border-gray-800" />

            <h2 id="what-keeps-top-ranking">이 선수가 세계 상위권을 지키는 힘은 무엇인가?</h2>
            <p>프리츠가 세계 상위권을 유지하는 힘은 직선적 포핸드와 안정된 서브, 그리고 계산된 코스 선택에서 나온다.</p>
            ...
            <div className="not-prose my-4 p-4 bg-purple-50/50 dark:bg-purple-950/20 rounded-lg border-l-2 border-purple-400 dark:border-purple-600">
              <p className="text-sm text-gray-700 dark:text-gray-300 mb-0">
                <strong className="text-purple-700 dark:text-purple-300">직선적 포핸드</strong>가 가장 큰 무기. 단단한 포핸드와 계산된 각으로 큰 코트의 시간을 단축한다.
              </p>
            </div>
            <p>직선적 포핸드가 가장 큰 무기다. 단단한 포핸드와 계산된 각으로 큰 코트의 시간을 단축한다. 포핸드는 넓은 궤도와 직선적 압박으로 상대를 뒤로 밀어낸다. 백핸드는 양손의 견고함으로 라인을 얇게 긁어 각을 만든다. 서브는 와이드와 바디의 비율을 바꿔 첫 두 타구의 구조를 설계하고, 짧은 리턴이 오면 하프코트에서 한 걸음 전진해 발리로 결말을 재촉한다.</p>
            ...
            <p>코트 커버리지가 뛰어나다. 전진과 후퇴를 자유롭게 오간다. 짧은 볼이 보이면 하프코트에서 한 걸음 전진해 발리로 마무리한다. 장기전에서도 체력 관리가 뛰어나며, <strong className="bg-yellow-100 dark:bg-yellow-900/30 px-1 rounded">큰 코트의 시간을 단축하는 능력</strong>이 특징이다.</p>
            ...
            <div className="not-prose my-4 p-4 bg-green-50 dark:bg-green-950/20 border-l-4 border-green-500 rounded-r-lg shadow-sm">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-0">
                하드 코트에서 비교적 강한 편 · 직선의 파워 · 계산된 각
              </p>
            </div>
            <p>포인트 사이 그는 스트링을 훑고 두 번의 깊은 호흡으로 리듬을 고정한다. 스코어가 흔들려도 루틴의 길이를 바꾸지 않아 심박이 급등하지 않는다. 하드 코트에서는 직선적 압박으로 초반 속도를 통제하고, 3~4포인트 구간에서만 라인을 얇게 써 승부수를 던진다. 직선의 파워와 계산된 각이 특징이다.</p>
            ...
            <p>프리츠의 커리어를 상징하는 경기는 인디언 웰스 우승과 하드 마스터스 대회 우승이다.</p>
            ...
            <div className="not-prose my-4 p-5 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 rounded-xl border border-blue-200 dark:border-blue-800 shadow-md">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 mb-0">
                <strong>인디언 웰스</strong>에서 우승을 차지하며 마스터스 대회 챔피언이 되었다. 특히 결승전에서의 경기력은 그의 직선적 포핸드와 안정된 서브를 보여주는 대표적인 사례였다.
              </p>
            </div>
            <p>인디언 웰스에서 그는 우승을 차지하며 마스터스 대회 챔피언이 되었다. 이 경기에서 그는 직선적 포핸드와 안정된 서브로 상대를 압도했다. 특히 결승전에서의 경기력은 그의 기술적 완성도를 보여주는 대표적인 사례였다. 단단한 포핸드와 계산된 각, 그리고 중요한 포인트에서의 침착함이 모두 어우러진 경기였다.</p>

            <h3>하드 마스터스 대회 우승</h3>
            <p>하드 마스터스 대회에서 그는 우승을 거두며 하드 전문가로서의 입지를 확고히 했다. 이 경기에서 드러난 건 프리츠의 계산된 코스 선택이었다. 직선의 파워와 계산된 각이 한 경기에 응축되었다.</p>
            ...
            <blockquote className="not-prose my-4 p-5 bg-gradient-to-br from-gray-50 to-slate-50 dark:from-gray-900/50 dark:to-slate-900/50 border-l-4 border-gray-400 dark:border-gray-600 rounded-r-lg shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <p className="text-base text-gray-800 dark:text-gray-200 mb-0 italic leading-relaxed">
                  브레이크 포인트. 와이드 서브로 상대를 코트 밖으로 밀어낸 뒤, 직선적 포핸드를 몸 가까이 끌어당겨 다운더라인으로 단호히 접는다. 공이 사이드라인을 스치며 빠져나갈 때 관중의 탄성이 터지고, 그는 표정을 바꾸지 않은 채 같은 속도로 베이스라인을 밟는다. 직선의 파워와 계산된 각이 바로 이 순간이다.
                </p>
              </div>
            </blockquote>

            <h2 id="what-attracts-fans">팬들은 이 선수의 어떤 점에 끌릴까?</h2>
            <p>프리츠는 경기력뿐 아니라 차분함과 책임 덕분에 팬층이 두텁다.</p>

            <h3>코트 매너</h3>
            <p>그는 차분함과 책임을 중시한다. 인터뷰에서 준비와 팀을 먼저 언급한다. 잘된 날에는 계획이 맞았다고, 부족한 날에는 수정 항목이 분명하다고 말한다. 팬에겐 미소와 눈맞춤을, 코트 스태프에겐 짧은 감사를 잊지 않는다. 패배 뒤에도 책임의 방향을 자기 쪽으로 당기고, 승리한 날에는 스태프와 팬에게 차례로 감사를 전한다.</p>

            <h3>인터뷰 스타일</h3>
            <p>인터뷰는 담백하고 체계적이다. 준비와 팀을 먼저 언급하며, 자신의 성과보다는 상대의 강점을 인정하는 모습을 자주 보인다. 긴 일정 속에서도 회복·수면·식단의 루틴을 생활로 고정해 컨디션의 파고를 낮춘다. 작은 제스처와 정확한 선택이 모여 한 사람의 초상을 완성한다.</p>

            <h3>팀과의 관계</h3>
            <p>코치와 팀과의 관계에서도 신뢰가 두터운 것으로 알려져 있다. 오랜 기간 함께한 코치와의 유대는 그의 안정적인 경기력에 기여하고 있으며, 팀 전체가 하나의 목표를 향해 나아가는 모습을 보여준다. 미국의 훈련 문화는 하드의 전통을 현대적으로 번역하는 것을 중시하는데, 프리츠의 테니스에도 이 정신이 뚜렷하다.</p>

            <h2 id="recent-form">요즘 테일러 프리츠의 경기력 흐름은 어떤가?</h2>
            <p>최근 몇 시즌 동안 프리츠는 하드 시즌에서 특히 강력한 모습을 보여주고 있으며, 큰 대회에서도 안정적인 경기력을 보여주고 있다.</p>
