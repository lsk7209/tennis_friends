...
2. 속성 추가 → URL 접두어: `https://tennisfrens.com`
3. 소유권 확인:
   - **HTML 메타 태그** 방식 선택
   - `content` 값 복사 → `.env.local`의 `GOOGLE_SITE_VERIFICATION`에 붙여넣기
   - 배포 후 "확인" 클릭
4. 사이트맵 제출: `https://tennisfrens.com/sitemap.xml`

### 3. 네이버 서치어드바이저 등록

1. https://searchadvisor.naver.com/ 접속
2. 사이트 추가 → `https://tennisfrens.com`
3. 소유권 확인:
   - **HTML 메타 태그** 방식 선택
   - `content` 값 복사 → `.env.local`의 `NAVER_SITE_VERIFICATION`에 붙여넣기
   - 배포 후 "확인" 클릭
4. 사이트맵 제출: `https://tennisfrens.com/sitemap-naver.xml`
...
### 인증 파일
- 구글: `https://tennisfrens.com/google<코드>.html`
- 네이버: `https://tennisfrens.com/naver<코드>.html`
- Bing: `https://tennisfrens.com/BingSiteAuth.xml`
