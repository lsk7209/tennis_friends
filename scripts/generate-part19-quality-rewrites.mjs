...
<p>${j(row.title, "을/를")} 검색하는 사람은 이미 비슷한 조언을 여러 번 들었을 가능성이 큽니다. 문제는 조언이 부족한 것이 아니라 경기 중 어느 장면에 적용해야 하는지 흐려지는 데 있습니다. 이 글은 ${main}, ${extended}, ${j(support, "을/를")} 한 번에 고치자는 글이 아닙니다. ${row.angle}에 맞춰 먼저 볼 신호, 경기 중 유지할 기준, 경기 후 남길 기록을 나눕니다. 기본 규칙과 안전 기준은 ${j(sources, "을/를")} 참고했고, 실제 동호회 경기와 레슨에서 바로 확인할 수 있는 표현으로 정리했습니다.</p>
...
<h2>${h2s[0]}</h2>
<p>${j(main, "은/는")} 결과 이름처럼 보이지만 실제로는 준비 위치, 타이밍, 몸 상태, 장비 상태, 상대 선택이 함께 섞인 신호입니다. 그래서 첫 단계는 문제를 크게 부르는 것이 아니라 직전 장면을 작게 나누는 일입니다. 공이 짧아졌다면 라켓 면이 문제였는지, 첫 발이 늦었는지, 상대 공이 더 깊었는지, 혹은 이미 체력이 떨어졌는지 구분해야 합니다. 이 구분이 되면 같은 실수라도 다음 행동이 달라집니다.</p>
...
<ul><li>${j(main, "이/가")} 나온 점수와 위치를 함께 적습니다.</li><li>${j(extended, "은/는")} 성공 여부보다 반복 가능한 준비 동작으로 봅니다.</li><li>${j(support, "은/는")} 다음 경기에서 확인할 문장 하나로 압축합니다.</li><li>통증, 미끄럼, 장비 이상처럼 안전과 관련된 신호는 기술 목표보다 먼저 멈춰 확인합니다.</li></ul>
<h2>${h2s[1]}</h2>
<p>${j(extended, "을/를")} 기준으로 볼 때 좋은 신호와 수정 신호를 분리해야 합니다. 좋은 신호는 다음 선택을 안정시키는 정보입니다. 수정 신호는 방금 점수를 잃었더라도 같은 실수가 왜 반복되는지 알려주는 정보입니다. 동호회 경기에서는 결과 감정이 강해서 이 구분이 쉽게 무너집니다. 이겼지만 위험 선택이 반복됐다면 연습 주제가 남아 있고, 졌지만 기준을 지켰다면 다음 경기에서 유지할 근거가 생깁니다.</p>
${buildTable(row)}
<p>이 표는 경기 직후 3분 안에 채울 정도로 짧아야 합니다. 길어지면 기록이 부담이 되고 결국 지속되지 않습니다. ${j(main, "은/는")} 장면 이름, ${j(extended, "은/는")} 판단 기준, ${j(support, "은/는")} 다음 확인 문장으로 남기세요. 세 줄만 있어도 레슨에서 질문할 때 "왜 안 되나요"가 아니라 "이 장면에서 제 준비가 늦은지 봐주세요"처럼 구체적인 피드백을 받을 수 있습니다.</p>
<h2>${h2s[2]}</h2>
