...
const openings = [
  (title, reader, angle) => `${j(title, "을/를")} 찾는 사람은 보통 이미 한두 번 같은 장면을 겪었습니다. ${reader}에게 필요한 것은 더 많은 조언이 아니라 ${angle}입니다.`,
  (title, reader, angle) => `${reader}라면 ${j(title, "은/는")} 단순한 팁 목록으로 끝나면 안 됩니다. 이 글은 ${angle}에 맞춰 판단 순서를 좁힙니다.`,
  (title, reader, angle) => `${title}의 핵심은 정답을 외우는 데 있지 않습니다. ${j(reader, "이/가")} 코트에서 다시 확인할 수 있도록 ${j(angle, "으로/로")} 정리합니다.`,
  (title, reader, angle) => `${j(reader, "이/가")} 가장 자주 놓치는 부분은 결과가 나온 뒤에야 원인을 찾는다는 점입니다. ${j(title, "은/는")} ${angle}에 초점을 둡니다.`,
];
...
<p>${opening} 검색에서 흔히 보이는 조언은 “자주 연습하라”, “기본기를 지켜라”처럼 맞지만 실행하기 어려운 말이 많습니다. 그래서 이 글은 ${main}, ${extended}, ${j(support, "을/를")} 분리해 코트에서 바로 체크할 수 있는 기준으로 바꿉니다. 기본 규칙과 코칭 관점은 ${j(sourceText, "을/를")} 기준으로 삼고, 동호인이 실제로 겪는 시간 부족, 장비 부담, 파트너와의 커뮤니케이션 문제까지 함께 반영했습니다.</p>
<p>먼저 목표를 작게 잡아야 합니다. 오늘 바꿀 것은 전체 경기력이 아니라 ${criterion}입니다. 이 기준을 세우면 실수 뒤에 “멘탈이 약했다” 또는 “운이 없었다”로 끝나지 않고, 다음 공에서 무엇을 조정할지 남습니다. 관련 도구와 보충 글은 ${j(links, "을/를")} 함께 확인하면 기록과 복습을 연결하기 쉽습니다.</p>
<h2>${h2[0]}</h2>
<p>${j(main, "은/는")} 결과보다 전조를 봐야 합니다. 공이 나간 순간만 보면 이미 늦습니다. 준비 위치가 늦었는지, 첫 발이 멈췄는지, 라켓을 잡는 힘이 갑자기 세졌는지처럼 실수 직전의 신호를 찾아야 합니다. ${j(reader, "은/는")} 보통 같은 신호를 여러 번 놓친 뒤에야 문제를 알아차립니다. 다음 경기에서는 한 포인트가 끝날 때마다 “방금 실수 전에 무엇이 먼저 흔들렸나”를 한 문장으로 남겨보세요.</p>
<aside style="border-left:4px solid ${accent.border};background:#ffffff;padding:14px 16px;margin:18px 0;border-radius:8px;"><strong>판단 박스</strong><br>${j(criterion, "을/를")} 확인하지 못했다면 기술을 더 추가하지 않는 편이 좋습니다. 기준이 없는 상태에서 새 팁을 넣으면 다음 경기에서 원인을 더 찾기 어려워집니다.</aside>
<ul><li>${j(main, "을/를")} 경기 중 부르는 짧은 단어로 바꿉니다.</li><li>${j(extended, "은/는")} 성공 여부가 아니라 반복 가능한 준비 동작으로 기록합니다.</li><li>${j(support, "은/는")} 다음 연습에서 확인할 장면 하나만 남깁니다.</li><li>통증, 장비 손상, 미끄러짐처럼 안전과 관련된 신호는 연습 목표보다 먼저 봅니다.</li></ul>
<h2>${h2[1]}</h2>
<p>${j(extended, "을/를")} 볼 때는 좋은 신호와 수정 신호를 분리해야 합니다. 좋은 신호는 몸의 균형이 남고, 다음 선택을 말로 설명할 수 있으며, 같은 상황에서 다시 시도할 수 있다는 점입니다. 수정 신호는 반대로 성공했는데도 이유를 모르거나, 실패 뒤에 같은 행동을 더 세게 반복하는 것입니다. 이 차이를 적어두면 코치에게 질문할 때도 “왜 안 되나요?”가 아니라 “이 장면에서 기준을 이렇게 잡아도 되나요?”라고 물을 수 있습니다.</p>
${table(tags, criterion)}
<p>표는 복잡하게 채우는 도구가 아닙니다. 경기 후 3분 안에 한 줄만 남겨도 충분합니다. 예를 들어 ${j(main, "이/가")} 흔들린 날에는 점수보다 준비 동작을 먼저 적고, ${j(extended, "이/가")} 맞았던 날에는 왜 맞았는지 조건을 적습니다. ${j(support, "은/는")} 다음 훈련에서 그대로 반복할 수 있어야 의미가 있습니다.</p>
<h2>${h2[2]}</h2>
<p>20분 루틴은 네 단계로 나누면 안정적입니다. 처음 5분은 몸풀기와 기준 확인, 다음 7분은 같은 장면 반복, 그다음 5분은 점수를 붙인 적용, 마지막 3분은 기록입니다. 이때 성공률만 보지 마세요. ${j(criterion, "이/가")} 지켜졌는지, 그리고 실패했을 때 같은 원인이 반복됐는지가 더 중요합니다.</p>
<ol><li>오늘 볼 기준을 ${main} 하나로 제한합니다.</li><li>${j(extended, "이/가")} 나타나는 장면을 일부러 만듭니다.</li><li>${j(support, "과/와")} 연결되는 실전 점수 상황을 붙입니다.</li><li>마지막에는 다음 경기에서 확인할 문장 하나만 남깁니다.</li></ol>
...
<details><summary>${j(main, "은/는")} 하루 만에 고칠 수 있나요?</summary><p>완전히 고치기는 어렵지만 한 경기에서 확인할 기준 하나를 줄이는 것은 가능합니다. 이 글의 목적도 완성보다 반복 가능한 판단 기준을 만드는 데 있습니다.</p></details>
<details><summary>${j(extended, "과/와")} ${support} 중 무엇을 먼저 봐야 하나요?</summary><p>경기 중에는 ${j(extended, "을/를")} 먼저 보고, 경기 후 복기에서 ${j(support, "을/를")} 확인하는 순서가 안전합니다. 동시에 두 가지를 바꾸면 원인을 찾기 어렵습니다.</p></details>
<details><summary>초보자도 이 기준을 써도 되나요?</summary><p>초보자일수록 기준을 줄이는 편이 좋습니다. 단, 통증이나 장비 문제처럼 안전과 관련된 신호는 연습보다 먼저 확인해야 합니다.</p></details>
<p>정리하면 ${title}의 핵심은 더 많은 조언을 모으는 것이 아니라, 다음 경기에서 확인할 기준을 줄이는 것입니다. ${j(main, "을/를")} 중심으로 ${extended}, ${j(support, "을/를")} 나눠 기록하면 실수의 원인이 선명해지고, 연습 시간도 덜 낭비됩니다.</p>`;
}
...
const contentEntries = rows
  .map((row, index) => {
    ...
    const faq = [
      { question: `${tags[0]}에서 가장 먼저 볼 기준은?`, answer: `${j(tags[1], "과/와")} ${j(tags[2], "을/를")} 동시에 바꾸지 말고, 경기 중에는 하나의 신호만 먼저 확인하는 편이 안전합니다.` },
    ...
    tags: ${esc(tags)},
    summary: ${esc(excerpt)},
    highlight: ${esc(`${title}의 핵심은 ${j(tags[0], "을/를")} ${j(tags[1], "과/와")} ${tags[2]} 기준으로 나눠 다음 경기에서 확인할 수 있게 만드는 것입니다.`)},
