  ...
  for (const topic of topics) {
    ...
    for (const variant of variants) {
      ...
      rows.push({
        ...
        excerpt: `${j(topic.tags[0], "은/는")} 무리한 교정보다 ${topic.tags[1]} 신호를 먼저 기록해야 안전합니다.`,
...
function buildContent(row, index) {
  ...
  return `<p>${j(row.title, "은/는")} ${j(main, "을/를")} 경기력 문제로만 보지 않고, 다음 경기까지 이어지는 몸 상태와 장비 상태의 신호로 읽기 위한 글입니다. 테니스프렌즈의 기준은 간단합니다. 아프거나 불안한 상태를 참고 밀어붙이는 것이 아니라, ${extended}, ${support}, ${j(row.intent, "을/를")} 기록 가능한 기준으로 바꾸는 것입니다. 이 글은 의학적 진단이 아니라 동호회 선수가 스스로 무리를 줄이고 전문가 상담이 필요한 상황을 놓치지 않도록 돕는 일반 가이드입니다.</p>
<p>${row.category} 주제는 과장되기 쉽습니다. 어떤 글은 한 가지 스트레칭으로 모든 문제가 해결된다고 말하고, 어떤 글은 장비 하나로 통증이 사라진다고 말합니다. 실제로는 그렇게 단순하지 않습니다. 경기 전 컨디션, 최근 훈련량, 코트 표면, 신발 마모, 수면, 수분, 그립 압력 같은 요소가 함께 작동합니다. 그래서 ${j(row.title, "은/는")} 해결책 목록보다 판단 순서를 먼저 제시합니다.</p>
...
<p>첫 기준은 "오늘 더 잘하기"가 아니라 "오늘 악화시키지 않기"입니다. ${j(main, "이/가")} 반복되면 기술 교정 전에 ${j(extended, "이/가")} 언제 나타나는지부터 봐야 합니다. 공을 세게 칠 때만 나타나는지, 서브 후에만 나타나는지, 사이드스텝 뒤에만 나타나는지, 더운 날 후반부에만 나타나는지 나누면 다음 선택이 쉬워집니다. 팔꿈치 통증처럼 특정 증상이 이어지는 경우에는 <a href="${source[1]}" target="_blank" rel="noopener noreferrer">${source[0]}</a> 같은 공신력 있는 자료를 확인하고, 증상이 계속되면 전문 평가를 받는 편이 안전합니다.</p>
<ul>
  <li>${main}: 오늘 관찰할 몸 또는 장비 신호를 하나로 줄입니다.</li>
  <li>${extended}: 언제, 어떤 동작 뒤에 나타나는지 기록합니다.</li>
  <li>${support}: 다음 경기에서 낮출 강도나 바꿀 루틴을 정합니다.</li>
</ul>
<h2>${j(mode, "으로/로")} 보는 경기 전후 루틴</h2>
<p>${j(row.title, "을/를")} 실제로 쓰려면 루틴이 길어지면 안 됩니다. 경기 전에는 2분만 투자해도 충분합니다. 신발 접지, 손목과 팔꿈치 느낌, 어깨 회전, 무릎 사이드스텝, 수분 상태, 수면 부족 여부 중 오늘의 주제 하나만 고릅니다. 경기 중에는 점수를 잃은 이유를 전부 분석하지 말고 ${extended} 신호가 있었는지 확인합니다. 경기 후에는 ${support} 기준으로 다음 훈련 강도를 낮출지, 장비를 점검할지, 휴식을 늘릴지 결정합니다.</p>
<table>
  <thead><tr><th>구간</th><th>확인 질문</th><th>실행 기준</th></tr></thead>
  <tbody>
    <tr><td>경기 전</td><td>${j(main, "이/가")} 오늘 이미 느껴지는가?</td><td>강도 10~20% 낮추기</td></tr>
    <tr><td>경기 중</td><td>${j(extended, "이/가")} 특정 동작 뒤 반복되는가?</td><td>무리한 한 방 줄이기</td></tr>
    <tr><td>경기 후</td><td>${support} 기준으로 다음 선택이 보이는가?</td><td>기록, 휴식, 점검 중 하나 선택</td></tr>
  </tbody>
</table>
<p>이 표의 목적은 불안을 키우는 것이 아닙니다. 오히려 막연한 걱정을 줄이는 데 있습니다. 무엇을 봐야 할지 정해지면 불필요한 검색을 줄이고, 같은 문제를 다음 경기에서 더 정확히 확인할 수 있습니다. 특히 ${j(main, "은/는")} 경기력이 좋았던 날에도 기록해 두면 좋습니다. 문제가 없던 날의 루틴을 알아야 문제가 생긴 날과 비교할 수 있습니다.</p>
<h2>${main}에서 자주 생기는 오해</h2>
<p>첫 번째 오해는 통증이나 피로를 정신력 문제로 보는 것입니다. 컨디션 신호는 의지와 별개로 다뤄야 합니다. 두 번째 오해는 장비나 루틴을 한 번에 많이 바꾸는 것입니다. 신발, 스트링, 그립, 워밍업, 수면 패턴을 동시에 바꾸면 무엇이 효과를 냈는지 알 수 없습니다. 세 번째 오해는 경기 후 기록을 남기지 않는 것입니다. 몸은 기억하지만 검색 기록은 금방 사라집니다. ${j(extended, "과/와")} ${j(support, "을/를")} 한 줄로 남겨야 다음 선택이 선명해집니다.</p>
...
<h2>초보와 중급자의 차이</h2>
<p>초보자는 ${j(row.title, "을/를")} 단일 정답으로 찾으려는 경우가 많습니다. 하지만 중급자는 같은 신호라도 경기 상황에 따라 다르게 조절합니다. 예를 들어 ${j(main, "이/가")} 있을 때 초보자는 무조건 쉬거나 무조건 참고 치려고 합니다. 중급자는 먼저 강도를 낮추고, 특정 동작을 줄이고, 다음 경기 전까지 회복 여부를 확인합니다. 차이는 지식의 양보다 기록의 정확도에서 나옵니다.</p>
<ol>
  <li>오늘 가장 선명한 신호 하나만 고릅니다.</li>
  <li>${j(extended, "이/가")} 나온 시점과 동작을 적습니다.</li>
  <li>${support} 기준으로 다음 경기 전 조절 항목을 정합니다.</li>
  <li>증상이 반복되거나 악화되면 훈련보다 상담을 우선합니다.</li>
</ol>
<h2>경기 후 3분 기록법</h2>
<p>경기 후 기록은 길 필요가 없습니다. 첫 줄에는 ${j(main, "이/가")} 있었는지 적습니다. 둘째 줄에는 ${j(extended, "이/가")} 언제 나타났는지 적습니다. 셋째 줄에는 ${j(support, "을/를")} 기준으로 다음 행동을 씁니다. 예를 들어 "후반 서브에서 팔꿈치 바깥쪽 당김, 다음 경기 전 서브 개수 제한"처럼 짧게 쓰면 충분합니다. 이 기록이 쌓이면 자신에게 맞는 회복 속도와 위험 신호가 보입니다.</p>
<details><summary>${j(row.title, "은/는")} 아프지 않아도 필요할까요?</summary><p>필요합니다. 문제가 없던 날의 기준이 있어야 컨디션이 떨어진 날과 비교할 수 있습니다.</p></details>
<details><summary>${j(extended, "이/가")} 계속되면 어떻게 해야 하나요?</summary><p>강도를 낮추고 반복 동작을 줄이세요. 통증, 붓기, 힘 빠짐, 감각 이상이 이어지면 전문가 상담을 우선하는 편이 안전합니다.</p></details>
<details><summary>${j(support, "은/는")} 얼마나 자세히 써야 하나요?</summary><p>세 줄이면 충분합니다. 장면, 신호, 다음 행동만 남기면 다음 경기 전 확인하기 쉽습니다.</p></details>
<p>정리하면 ${j(row.title, "은/는")} 무리하게 버티기 위한 글이 아니라, ${main}, ${extended}, ${j(support, "을/를")} 통해 경기와 회복을 같이 관리하기 위한 기준입니다. 오늘 한 가지 신호를 고르고, 경기 중 확인하고, 경기 후 한 줄로 남기세요. 그 기록이 다음 연습의 강도와 장비 선택, 휴식 판단을 더 안전하게 만들어 줍니다.</p>`;
}

const contentEntries = rows
  .map((row, index) => {
    const faq = [
      { question: `${j(row.tags[0], "은/는")} 언제 기록해야 하나요?`, answer: `경기 전 느낌, 경기 중 ${row.tags[1]} 발생 시점, 경기 후 ${row.tags[2]} 기준을 세 줄로 남기면 충분합니다.` },
    ...
    highlight: ${esc(`${j(row.title, "은/는")} ${row.tags[0]}, ${row.tags[1]}, ${j(row.tags[2], "을/를")} 안전한 경기 기록 기준으로 바꾸는 글입니다.`)},
