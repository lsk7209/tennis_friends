...
function buildContent(row, index) {
  ...
  return `<p>${j(row.title, "은/는")} ${j(row.main, "을/를")} 감각적인 조언이 아니라 경기에서 다시 확인할 수 있는 기준으로 바꾸는 글입니다. ${j(row.extended, "과/와")} ${j(row.support, "이/가")} 함께 흔들리면 선수는 보통 더 세게 치거나 더 많이 연습하려고 합니다. 하지만 실제 개선은 한 장면을 고르고, 같은 언어로 기록하고, 다음 경기 전 다시 읽는 데서 시작됩니다.</p>
<p>이 글의 관점은 단순합니다. ${j(row.main, "이/가")} 흔들릴 때 바로 결론을 내리지 말고 ${row.extended} 신호를 먼저 봅니다. 그다음 ${j(row.support, "을/를")} 한 줄로 남깁니다. 기본 규칙과 훈련 원칙은 <a href="${source[1]}" target="_blank" rel="noopener noreferrer">${source[0]}</a> 같은 공식 자료를 기준으로 삼고, 동호회 환경에서는 더 짧고 반복 가능한 루틴으로 줄여 씁니다.</p>
<aside class="${accent} border-l-4 p-4 my-6"><strong>오늘의 한 줄</strong><br />${j(row.main, "은/는")} 의지 문제가 아니라 기준 문제입니다. ${j(row.extended, "을/를")} 먼저 확인하고 ${row.support} 한 줄을 남기면 다음 경기에서 바로 복구할 수 있습니다.</aside>
<h2>${j(row.main, "을/를")} 볼 때 첫 기준</h2>
<p>${situation}에서는 판단 속도가 빨라진 것처럼 느껴지지만 실제로는 확인 순서가 사라진 경우가 많습니다. 첫 기준은 결과가 아닙니다. 성공했는지 실패했는지보다 어떤 신호를 보고 움직였는지가 중요합니다. 그래서 ${j(row.extended, "을/를")} 기준으로 잡고, ${j(row.support, "은/는")} 다음 행동을 남기는 기록 단위로 둡니다.</p>
<ul>
...
<h2>${j(row.extended, "과/와")} ${j(row.support, "을/를")} 연결하는 순서</h2>
<p>첫째, 경기 전에는 ${j(row.main, "이/가")} 나올 가능성이 큰 상황을 하나 고릅니다. 둘째, 포인트 중에는 몸의 반응보다 ${row.extended} 신호를 먼저 확인합니다. 셋째, 포인트 뒤에는 ${j(row.support, "을/를")} 기준으로 짧게 복기합니다. 넷째, 다음 연습의 첫 20분에 같은 장면을 다시 배치합니다.</p>
<table>
  <thead><tr><th>구간</th><th>질문</th><th>기록</th></tr></thead>
  <tbody>
    <tr><td>경기 전</td><td>${j(row.main, "이/가")} 흔들릴 조건은?</td><td>예상 장면 1개</td></tr>
    <tr><td>포인트 중</td><td>${row.extended} 신호를 봤나?</td><td>멈춤 기준 1개</td></tr>
    <tr><td>경기 후</td><td>${j(row.support, "으로/로")} 남길 행동은?</td><td>다음 행동 1문장</td></tr>
  </tbody>
</table>
<p>이 표를 쓰면 복기가 길어지지 않습니다. 긴 복기는 그날 기분을 담기 쉽고, 짧은 복기는 다음 행동으로 이어지기 쉽습니다. ${j(row.main, "은/는")} 특히 반복성이 중요합니다. 같은 장면을 같은 문장으로 남겨야 변화가 보입니다.</p>
<h2>자주 틀리는 판단</h2>
<p>첫 번째 실수는 ${j(row.main, "을/를")} 전체 실력 문제로 확대하는 것입니다. 한 장면에서 나온 흔들림을 전부 고치려 하면 연습이 흐려집니다. 두 번째 실수는 ${j(row.extended, "을/를")} 보지 않고 결과만 복기하는 것입니다. 세 번째 실수는 ${row.support} 없이 다음부터 조심하겠다고만 적는 것입니다. 이 세 가지가 쌓이면 연습량은 늘어도 경기 내용은 잘 바뀌지 않습니다.</p>
...
<h2>초보와 중급자의 차이</h2>
<p>초보자는 보통 ${j(row.main, "이/가")} 실패한 뒤에야 문제를 알아차립니다. 중급자는 실패 직전의 신호를 봅니다. ${j(row.extended, "이/가")} 바로 그 신호입니다. 하지만 중급자도 기록을 남기지 않으면 다음 경기에서 같은 장면을 다시 놓칩니다. 그래서 ${j(row.support, "을/를")} 짧게 남기는 습관이 필요합니다.</p>
<ol>
  <li>오늘 흔들린 장면 하나를 선택합니다.</li>
  <li>${j(row.extended, "이/가")} 보였는지 확인합니다.</li>
  <li>${j(row.support, "으로/로")} 다음 행동을 씁니다.</li>
...
<h2>3분 복기 예시</h2>
<p>복기는 세 줄이면 충분합니다. 첫 줄은 ${j(row.main, "이/가")} 나온 장면, 둘째 줄은 ${j(row.extended, "을/를")} 봤는지 여부, 셋째 줄은 ${j(row.support, "으로/로")} 다음 행동을 쓰는 방식입니다. 이렇게 남기면 다음 연습이 바로 정해집니다. "못했다"가 아니라 "어떤 신호를 놓쳤다"가 남아야 합니다.</p>
<details><summary>${j(row.main, "은/는")} 매번 기록해야 하나요?</summary><p>아닙니다. 같은 문제가 두 번 이상 반복된 날에만 짧게 남겨도 충분합니다.</p></details>
<details><summary>${j(row.extended, "이/가")} 잘 보이지 않으면 어떻게 하나요?</summary><p>점수 압박이 낮은 연습 랠리에서 먼저 확인하세요. 경기에서는 하나의 신호만 꺼내면 됩니다.</p></details>
<details><summary>${j(row.support, "은/는")} 얼마나 구체적이어야 하나요?</summary><p>다음 경기 전 바로 읽고 행동할 수 있는 한 문장이면 됩니다. 길수록 다시 쓰기 어렵습니다.</p></details>
<p>정리하면 ${row.title}의 핵심은 ${row.main}, ${row.extended}, ${j(row.support, "을/를")} 한 문장으로 묶는 것입니다. 오늘 하나의 장면만 고르고, 다음 경기 전 같은 문장으로 확인하세요. 그 반복이 실제 경기력과 콘텐츠 품질을 함께 올립니다.</p>`;
}
...
const contentEntries = rows
  .map((row, index) => {
    const faq = [
      { question: `${j(row.main, "은/는")} 어디서부터 점검하나요?`, answer: `${row.extended} 신호를 먼저 보고 ${row.support} 한 줄로 남기면 됩니다.` },
    ...
    highlight: ${esc(`${j(row.main, "을/를")} ${row.extended}, ${row.support} 기준으로 줄여 다음 경기에서 반복할 수 있게 정리합니다.`)},
