...
const variants = [
  ...
  },
  {
    ...
    angle: (stem) => `${j(stem, "을/를")} 경기 후 복기 가능한 한 줄 기록으로 바꾸기`,
  },
  {
...
<p>${j(row.title, "을/를")} 찾는 사람은 이미 비슷한 장면에서 여러 번 흔들렸을 가능성이 큽니다. 문제는 조언이 부족한 것이 아니라 경기 중 어떤 신호를 먼저 봐야 할지 흐려지는 데 있습니다. 이 글은 ${main}, ${extended}, ${j(support, "을/를")} 한꺼번에 고치자는 글이 아닙니다. ${row.angle}에 맞춰 먼저 볼 기준과 나중에 남길 기록을 분리합니다. 기본 규칙과 훈련 원칙은 ${j(sourceLinks(), "을/를")} 참고했고, 실제 동호회 경기에서 바로 확인 가능한 표현으로 정리했습니다.</p>
...
<h2>${h2s[0]}</h2>
<p>${j(main, "은/는")} 결과 이름처럼 보이지만 실제로는 준비 위치, 타이밍, 상대 공의 깊이, 몸 상태가 함께 섞인 신호입니다. 그래서 첫 단계는 큰 문제 이름을 붙이는 것이 아니라 직전 장면을 작게 나누는 일입니다. 준비가 늦었는지, 첫 발이 멈췄는지, 라켓 면이 열렸는지, 호흡이 끊겼는지를 나눠 봐야 합니다. 이 구분이 되면 같은 실수라도 다음 행동이 달라집니다.</p>
...
<ul><li>${j(main, "이/가")} 나온 점수와 위치를 함께 적습니다.</li><li>${j(extended, "은/는")} 성공 여부보다 반복 가능한 준비 동작으로 봅니다.</li><li>${j(support, "은/는")} 다음 경기에서 확인할 문장 하나로 압축합니다.</li><li>통증, 미끄럼, 장비 이상처럼 안전과 관련된 신호는 훈련 목표보다 먼저 확인합니다.</li></ul>
<h2>${h2s[1]}</h2>
<p>${j(extended, "을/를")} 경기 중에 확인하려면 좋은 신호와 수정 신호를 분리해야 합니다. 좋은 신호는 다음 선택을 안정시키는 정보입니다. 수정 신호는 방금 점수를 잃었더라도 같은 실수가 왜 반복되는지 알려주는 정보입니다. 동호회 경기에서는 결과 감정이 강해서 이 구분이 쉽게 무너집니다. 이겼지만 위험 선택이 반복됐다면 연습 주제가 남아 있고, 졌지만 기준을 지켰다면 다음 경기에서 유지할 근거가 생깁니다.</p>
${buildTable(row)}
<p>표는 경기 직후 3분 안에 채울 정도로 짧아야 합니다. 길게 쓰면 기록 자체가 부담이 되고 결국 지속되지 않습니다. ${j(main, "은/는")} 장면 이름, ${j(extended, "은/는")} 판단 기준, ${j(support, "은/는")} 다음 확인 문장으로 남기세요. 세 줄만 있어도 레슨에서 질문할 때 "왜 안 되나요"가 아니라 "이 장면에서 제 준비가 늦은지 봐주세요"처럼 구체적인 피드백을 받을 수 있습니다.</p>
<h2>${h2s[2]}</h2>
<p>20분 루틴은 네 구간이면 충분합니다. 처음 5분은 몸과 장비 상태를 확인합니다. 다음 7분은 같은 장면을 강도 없이 반복합니다. 그다음 5분은 실제 점수 상황을 붙입니다. 마지막 3분은 기록입니다. 이때 성공률을 길게 계산할 필요는 없습니다. ${j(support, "이/가")} 다음 경기에서 다시 확인 가능한 문장으로 남았는지가 더 중요합니다.</p>
<ol><li>오늘 볼 기준을 ${main} 하나로 제한합니다.</li><li>${j(extended, "이/가")} 드러나는 장면을 두 가지 이하로 고릅니다.</li><li>${j(support, "과/와")} 연결되는 실제 점수 상황을 붙입니다.</li><li>마지막에는 다음 경기 전 읽을 문장 하나만 남깁니다.</li></ol>
...
<details><summary>${j(main, "은/는")} 하루 만에 바꿀 수 있나요?</summary><p>완전히 바꾸기는 어렵지만 한 가지 신호를 알아차리는 것은 가능합니다. 오늘은 성공률보다 같은 장면에서 같은 기준을 적용했는지 확인하세요.</p></details>
<details><summary>${j(extended, "과/와")} ${support} 중 무엇을 먼저 봐야 하나요?</summary><p>경기 중에는 ${j(extended, "을/를")} 먼저 보고, 경기 후에는 ${j(support, "을/를")} 확인하는 순서가 좋습니다. 동시에 둘 다 바꾸려 하면 원인 추적이 흐려집니다.</p></details>
...
<p>정리하면 ${row.title}의 목표는 더 많은 조언을 모으는 것이 아니라 ${main}, ${extended}, ${j(support, "을/를")} 통해 다음 행동을 좁히는 것입니다. 경기 전에는 기준을 하나만 고르고, 경기 중에는 신호를 보고, 경기 후에는 다음 확인 문장을 남기세요. 이 흐름이 쌓이면 검색으로 얻은 정보가 실제 실력 변화로 이어집니다.</p>`;
}

const contentEntries = selectedRows
  .map((row, index) => {
    const faq = [
      { question: `${row.tags[0]}에서 가장 먼저 볼 기준은?`, answer: `${j(row.tags[1], "과/와")} ${j(row.tags[2], "을/를")} 동시에 바꾸지 말고 경기 중 확인 가능한 신호 하나부터 봅니다.` },
    ...
    highlight: ${esc(`${j(row.title, "은/는")} ${row.tags[0]}, ${row.tags[1]}, ${j(row.tags[2], "을/를")} 분리해 다음 경기에서 확인 가능한 기준으로 바꾸는 글입니다.`)},
