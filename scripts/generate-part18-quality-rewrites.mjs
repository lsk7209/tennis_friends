...
<p>${j(row.title, "을/를")} 찾는 사람은 대개 이미 한두 번 같은 장면에서 막힌 경험이 있습니다. 검색 결과에는 비슷한 조언이 많지만 실제 코트에서는 무엇부터 바꿔야 할지 흐려집니다. 이 글은 ${main}, ${extended}, ${j(support, "을/를")} 한꺼번에 고치자는 이야기가 아닙니다. ${row.angle}에 맞춰 먼저 볼 신호와 나중에 남길 기록을 분리합니다. 기본 규칙과 훈련 원칙은 ${j(sources, "을/를")} 기준으로 삼고, 동호회 경기와 레슨에서 바로 확인할 수 있는 장면만 골랐습니다.</p>
...
<h2>${h2s[0]}</h2>
<p>${j(main, "은/는")} 단어만 보면 단순해 보이지만 실제로는 여러 신호가 섞여 있습니다. 예를 들어 공이 짧아지는 문제와 몸이 급해지는 문제는 같은 결과를 만들 수 있습니다. 그래서 첫 번째 단계는 결과 이름을 붙이는 것이 아니라 직전 장면을 관찰하는 것입니다. 준비 위치가 늦었는지, 첫 발이 멈췄는지, 라켓 면이 열렸는지, 호흡이 끊겼는지를 나눠 봐야 합니다. 이렇게 보면 같은 실수라도 수정 방법이 달라집니다.</p>
...
<ul><li>${j(main, "이/가")} 나온 장면을 점수, 위치, 상대 공의 깊이와 함께 적습니다.</li><li>${j(extended, "은/는")} 성공 여부보다 반복 가능한 준비 동작으로 평가합니다.</li><li>${j(support, "은/는")} 다음 경기에서 확인할 한 문장으로 압축합니다.</li><li>통증, 장비 이상, 미끄럼처럼 안전과 관련된 신호는 훈련 목표보다 먼저 멈춰 확인합니다.</li></ul>
<h2>${h2s[1]}</h2>
<p>${j(extended, "을/를")} 경기 안에서 확인하려면 좋은 신호와 수정 신호를 분리해야 합니다. 좋은 신호는 다음 행동을 안정시키는 정보입니다. 수정 신호는 방금 점수를 잃었더라도 왜 같은 선택이 반복되는지 알려주는 정보입니다. 동호회 경기에서는 결과가 감정을 크게 흔들기 때문에 이 구분이 특히 중요합니다. 이겼는데도 같은 위험 선택이 반복됐다면 연습 주제가 남아 있고, 졌더라도 기준이 지켜졌다면 다음 경기에서 유지할 근거가 생깁니다.</p>
${buildTable(row)}
<p>표는 경기 직후 3분 안에 채우는 정도면 충분합니다. 길게 쓰면 다음부터 기록 자체가 부담이 됩니다. ${j(main, "은/는")} 장면 이름, ${j(extended, "은/는")} 기준, ${j(support, "은/는")} 다음 확인 문장으로 적으세요. 이 세 줄이 있으면 레슨에서 질문할 때도 "왜 안 되나요"가 아니라 "이 장면에서 제 준비가 늦은지 봐주세요"처럼 구체적으로 물을 수 있습니다.</p>
<h2>${h2s[2]}</h2>
<p>20분 실행 순서는 네 구간으로 나누는 편이 안전합니다. 처음 5분은 몸 상태와 장비 상태를 확인합니다. 다음 7분은 같은 장면을 반복하되 강도를 올리지 않습니다. 그다음 5분은 실제 점수 상황을 붙입니다. 마지막 3분은 기록입니다. 이때 성공률을 길게 계산할 필요는 없습니다. ${j(support, "이/가")} 다음 경기에서 다시 확인 가능한 문장으로 남았는지가 더 중요합니다.</p>
...
<details><summary>${j(main, "은/는")} 하루 만에 고칠 수 있나요?</summary><p>완전히 고치기는 어렵지만 한 가지 신호를 알아차리는 것은 가능합니다. 오늘은 성공률보다 같은 장면에서 같은 기준을 적용했는지를 확인하세요.</p></details>
<details><summary>${j(extended, "과/와")} ${support} 중 무엇을 먼저 봐야 하나요?</summary><p>경기 중에는 ${j(extended, "을/를")} 먼저 보고, 경기 후 복기에서는 ${j(support, "을/를")} 확인하는 순서가 좋습니다. 동시에 둘 다 바꾸려 하면 원인 추적이 흐려집니다.</p></details>
