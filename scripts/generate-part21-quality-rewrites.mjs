  ...
  for (const topic of topics) {
    ...
    for (const variant of variants) {
      ...
      rows.push({
        ...
        excerpt: `${j(topic.tags[0], "은/는")} 감각보다 ${topic.tags[1]} 기준을 먼저 정해야 흔들림이 줄어듭니다.`,
...
function buildContent(row, index) {
  ...
  return `<p>${j(row.title, "은/는")} ${j(main, "을/를")} 말로만 이해하지 않고 경기 중 한 번 더 재현 가능한 기준으로 바꾸기 위한 글입니다. ${row.category} 글은 보통 멋진 전술 이름이나 장비 스펙부터 설명하지만, 동호회 경기에서 결과를 바꾸는 부분은 더 작습니다. 어느 순간에 보고, 어떤 신호가 나오면 멈추고, 다음 포인트에서 무엇을 반복할지 정해야 합니다. 그래서 이 글은 ${extended}, ${support}, ${j(row.intent, "을/를")} 한 번에 묶어 설명합니다.</p>
<p>테니스프렌즈 페르소나는 초보와 중급자가 같은 말을 다르게 받아들이는 지점을 중요하게 봅니다. 예를 들어 ${j(main, "을/를")} 잘한다고 해도 파트너와 기준이 다르면 복식에서는 빈 공간이 생기고, 장비에서는 좋은 스펙이 오히려 손 부담으로 돌아옵니다. 반대로 기준을 너무 많이 잡으면 경기 중에는 아무것도 실행하지 못합니다. 이 글의 핵심은 하나입니다. 오늘 경기나 연습에서 바로 확인할 신호를 세 개 이하로 줄이는 것입니다.</p>
<aside class="${accent} border-l-4 p-4 my-6"><strong>오늘의 기준</strong><br />${j(row.title, "은/는")} 성공률을 높이는 주문이 아니라, ${j(main, "이/가")} 흔들릴 때 다시 돌아갈 체크 포인트입니다. 먼저 ${j(extended, "을/를")} 관찰하고, 그 다음 ${j(support, "을/를")} 기록하세요.</aside>
<h2>${j(row.title, "을/를")} 보기 전 정해야 할 한 문장</h2>
<p>시작 문장은 짧아야 합니다. "${j(main, "을/를")} 잘하자"는 문장은 너무 넓습니다. 대신 "${j(extended, "이/가")} 보이면 ${j(support, "을/를")} 먼저 확인한다"처럼 행동이 보이는 문장으로 바꾸세요. 전술과 장비 모두 이 방식이 유효합니다. 복식이라면 파트너가 들을 수 있는 말이어야 하고, 장비라면 다음 교체나 조정 때 다시 확인할 수 있어야 합니다. ITF 규정처럼 경기의 큰 틀은 명확하지만, 동호회 현장에서는 그 안에서 자신만의 판단 순서를 만드는 일이 중요합니다. 참고 규정과 기본 팁은 <a href="${source[1]}" target="_blank" rel="noopener noreferrer">${source[0]}</a>에서 확인할 수 있습니다.</p>
<ul>
  <li>${main}: 오늘 바꿀 주제 하나만 고릅니다.</li>
  <li>${extended}: 경기 중 관찰 가능한 신호로 바꿉니다.</li>
  <li>${support}: 다음 연습에서 반복할 기록 항목으로 남깁니다.</li>
</ul>
<h2>${j(mode, "으로/로")} 정리한 실행 순서</h2>
<p>${row.title}의 실행 순서는 상황을 복잡하게 만들지 않는 쪽으로 설계해야 합니다. 첫 번째는 경기 전 점검입니다. 몸이 덜 풀렸거나 파트너와 말이 맞지 않거나 장비 상태가 애매하면, 기술 교정보다 조건 확인이 먼저입니다. 두 번째는 경기 중 신호입니다. 공 하나를 놓쳤을 때 원인을 전부 찾으려 하지 말고 ${j(extended, "이/가")} 무너졌는지만 봅니다. 세 번째는 경기 후 기록입니다. 결과보다 다음 경기에서 다시 써볼 문장을 남기는 것이 좋습니다.</p>
<table>
  <thead><tr><th>구간</th><th>확인 질문</th><th>남길 기록</th></tr></thead>
  <tbody>
    <tr><td>경기 전</td><td>${j(main, "을/를")} 오늘 적용할 상황이 분명한가?</td><td>적용 포인트 1개</td></tr>
...
<p>이 표를 쓰면 글을 읽고 끝나는 일이 줄어듭니다. ${j(row.category, "은/는")} 특히 경기장 밖의 이해와 경기장 안의 실행 사이에 차이가 큽니다. 머리로는 알고 있지만 몸이 늦거나, 장비 정보는 많지만 실제 손 느낌을 기록하지 않는 경우가 많습니다. 그래서 표의 세 번째 칸이 가장 중요합니다. 다음 연습 20분 과제까지 내려와야 검색으로 얻은 정보가 실제 실력으로 이어집니다.</p>
<h2>${main}에서 자주 틀리는 판단</h2>
<p>첫 번째 실수는 결과만 보는 것입니다. 포인트를 땄는지 잃었는지만 보면 ${j(extended, "이/가")} 제대로 작동했는지 알 수 없습니다. 두 번째 실수는 한 번에 두 가지 이상 바꾸는 것입니다. 포칭 위치와 콜, 라켓 무게와 스트링, 신발 접지와 풋워크를 동시에 바꾸면 원인을 추적할 수 없습니다. 세 번째 실수는 파트너나 몸 상태를 빼놓는 것입니다. 복식 전술은 상대보다 우리 둘의 기준이 먼저이고, 장비 선택은 스펙보다 손과 발의 반응이 먼저입니다.</p>
...
<h2>초보와 중급자의 차이</h2>
<p>초보자는 ${j(row.title, "을/를")} 정답처럼 외우는 경향이 있습니다. 하지만 실제 경기에서는 상대의 속도, 코트 상태, 파트너 위치, 손의 피로도에 따라 같은 기준도 다르게 작동합니다. 중급자는 이 차이를 알고 있습니다. 그래서 모든 상황을 맞히려 하지 않고, 자신에게 반복되는 실패 신호를 먼저 찾습니다. ${main}도 마찬가지입니다. 오늘 필요한 것은 완벽한 설명이 아니라 반복되는 장면 하나를 줄이는 기준입니다.</p>
<ol>
  <li>오늘 가장 자주 나온 장면 하나를 고릅니다.</li>
  <li>${j(extended, "이/가")} 있었는지 없었는지 표시합니다.</li>
  <li>${j(support, "을/를")} 다음 경기 전 체크 문장으로 바꿉니다.</li>
  <li>다음 연습에서 같은 조건을 10회만 반복합니다.</li>
</ol>
<h2>경기 후 3분 기록법</h2>
<p>경기 후에는 길게 쓰지 않아도 됩니다. 첫 줄에는 ${j(main, "이/가")} 나온 장면을 적고, 둘째 줄에는 ${j(extended, "이/가")} 보였는지 적습니다. 셋째 줄에는 ${j(support, "을/를")} 기준으로 다음 행동을 씁니다. 이 세 줄이면 충분합니다. 중요한 것은 감정을 배제하는 것이 아니라 감정이 식기 전에 행동 기준으로 바꾸는 것입니다. "오늘 안 됐다"가 아니라 "다음에는 첫 신호를 파트너에게 먼저 말한다" 또는 "다음 교체 전 손 미끄럼을 기록한다"처럼 구체화해야 합니다.</p>
<details><summary>${j(row.title, "은/는")} 매 경기 적용해야 하나요?</summary><p>아닙니다. 같은 문제가 두 번 이상 반복될 때만 적용하세요. 모든 포인트에 적용하면 판단이 느려집니다.</p></details>
<details><summary>${j(extended, "과/와")} ${support} 중 무엇이 먼저인가요?</summary><p>경기 중에는 ${j(extended, "이/가")} 먼저이고, 경기 후에는 ${j(support, "이/가")} 먼저입니다. 관찰과 기록의 순서를 나눠야 실행이 쉬워집니다.</p></details>
...
<p>정리하면 ${j(row.title, "은/는")} 검색용 정보가 아니라 다음 경기의 행동 기준입니다. ${main}, ${extended}, ${j(support, "을/를")} 각각 따로 외우지 말고 한 장면으로 묶어 보세요. 그러면 글을 읽은 뒤 남는 것이 많아집니다. 오늘은 한 가지 신호만 고르고, 다음 경기에서는 그 신호가 다시 나오는지 확인하면 됩니다.</p>`;
}

const contentEntries = rows
  .map((row, index) => {
    const faq = [
      { question: `${j(row.tags[0], "은/는")} 어디서부터 봐야 하나요?`, answer: `${j(row.tags[1], "을/를")} 먼저 보고 ${j(row.tags[2], "은/는")} 경기 후 기록으로 남기는 순서가 좋습니다.` },
    ...
    highlight: ${esc(`${j(row.title, "은/는")} ${row.tags[0]}, ${row.tags[1]}, ${j(row.tags[2], "을/를")} 경기 중 확인 가능한 기준으로 바꾸는 글입니다.`)},
