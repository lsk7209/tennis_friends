  ...
  for (const topic of topics) {
    ...
    for (const variant of variants) {
      ...
      rows.push({
        ...
        excerpt: `${j(topic.tags[0], "은/는")} 결과보다 ${topic.tags[1]} 기준을 먼저 남겨야 다음 선택이 선명합니다.`,
...
function buildContent(row, index) {
  ...
  return `<p>${j(row.title, "은/는")} ${j(main, "을/를")} 막연한 감각이 아니라 다음 경기에서 다시 확인할 수 있는 기준으로 바꾸는 글입니다. ${j(row.category, "은/는")} 경기 후에 생각하면 쉬워 보이지만, 실제 포인트 안에서는 시간이 거의 없습니다. 그래서 이 글은 ${extended}, ${support}, ${j(row.intent, "을/를")} 한 줄 기록과 한 가지 행동으로 줄이는 방식에 집중합니다.</p>
<p>동호회 테니스에서 가장 자주 생기는 문제는 원인을 너무 많이 찾는 것입니다. 실점 하나에 폼, 멘탈, 상대, 코트, 장비, 체력을 모두 연결하면 다음 연습이 흐려집니다. 반대로 ${j(main, "을/를")} 한 장면으로 줄이면 복기가 쉬워집니다. 오늘은 전체 경기를 분석하지 않고, 가장 반복된 신호 하나만 고르는 방식으로 접근하세요.</p>
<aside class="${accent} border-l-4 p-4 my-6"><strong>오늘의 적용 기준</strong><br />${j(row.title, "은/는")} 멋진 해설보다 재현 가능한 판단을 남기는 것이 핵심입니다. 경기 중에는 ${extended}만 보고, 경기 후에는 ${j(support, "으로/로")} 다음 과제를 정하세요.</aside>
<h2>${j(row.title, "을/를")} 시작하는 한 문장</h2>
<p>첫 문장은 짧아야 합니다. "${j(main, "을/를")} 잘하자"는 문장은 행동으로 이어지지 않습니다. 대신 "${j(extended, "이/가")} 보이면 ${j(support, "을/를")} 먼저 확인한다"처럼 말해야 합니다. 경기 영상, 포핸드 실수, 브레이크 포인트, 야간 코트, 바람, 클레이, 잔디, 실내 하드, 매치포인트 모두 같은 원칙이 적용됩니다. 경기 규칙과 운영의 큰 틀은 <a href="${source[1]}" target="_blank" rel="noopener noreferrer">${source[0]}</a> 같은 공식 자료를 참고하되, 현장 실행은 자신의 반복 장면에 맞게 좁히는 편이 좋습니다.</p>
<ul>
...
<h2>${mode} 실행 순서</h2>
<p>${row.title}의 실행 순서는 세 단계입니다. 첫째, 경기 전에 관찰 기준을 정합니다. 둘째, 경기 중에는 결과보다 신호를 봅니다. 셋째, 경기 후에는 다음 연습의 첫 20분으로 바꿉니다. 이 세 단계가 연결되면 검색으로 얻은 정보가 경기 루틴이 됩니다. 특히 ${j(main, "은/는")} 한 번의 성공보다 반복되는 실패 장면을 줄이는 데 더 큰 가치가 있습니다.</p>
<table>
  <thead><tr><th>구간</th><th>확인 질문</th><th>기록 문장</th></tr></thead>
  <tbody>
    <tr><td>경기 전</td><td>${j(main, "을/를")} 볼 상황이 분명한가?</td><td>오늘 볼 장면 1개</td></tr>
    <tr><td>경기 중</td><td>${j(extended, "이/가")} 실제로 반복됐는가?</td><td>성공/실패 장면</td></tr>
    <tr><td>경기 후</td><td>${j(support, "으로/로")} 다음 과제가 정해졌는가?</td><td>다음 20분 과제</td></tr>
  </tbody>
</table>
...
<p>첫 번째는 결과 편향입니다. 포인트를 땄다고 좋은 선택이었고, 포인트를 잃었다고 나쁜 선택이었다고 단정하면 학습이 느려집니다. 두 번째는 장면을 크게 잡는 것입니다. "오늘 전체적으로 안 됐다"는 기록은 쓸모가 적습니다. "두 번째 브레이크 포인트에서 리턴 위치가 뒤로 밀렸다"처럼 장면이 보여야 합니다. 세 번째는 다음 연습으로 연결하지 않는 것입니다. ${j(extended, "을/를")} 봤다면 ${j(support, "을/를")} 반드시 남겨야 합니다.</p>
...
<h2>초보와 중급자의 차이</h2>
<p>초보자는 ${j(row.title, "을/를")} 정답 목록으로 찾는 경우가 많습니다. 중급자는 상황을 먼저 나눕니다. 같은 ${main}이라도 초반과 후반, 실내와 실외, 바람이 있는 날과 없는 날, 매치포인트와 일반 포인트는 다르게 봐야 합니다. 차이는 복잡한 지식이 아니라 기준을 바꾸는 능력입니다. 그래서 오늘은 ${extended} 하나만 기준으로 삼고, 다음 경기에서 같은 기준이 다시 맞는지 확인하세요.</p>
<ol>
  <li>오늘 가장 반복된 장면 하나를 고릅니다.</li>
  <li>${j(extended, "이/가")} 있었는지 표시합니다.</li>
  <li>${j(support, "을/를")} 다음 20분 연습으로 바꿉니다.</li>
  <li>다음 경기에서 같은 장면을 다시 확인합니다.</li>
</ol>
<h2>경기 후 3분 기록법</h2>
<p>경기 후에는 세 줄이면 충분합니다. 첫 줄은 ${j(main, "이/가")} 나온 장면, 둘째 줄은 ${j(extended, "이/가")} 보였는지, 셋째 줄은 ${j(support, "으로/로")} 다음 행동을 씁니다. 길게 쓰면 다시 읽지 않습니다. 짧게 쓰면 다음 경기 전에 바로 확인할 수 있습니다. 이 기록이 쌓이면 자신이 흔들리는 패턴과 코트별 적응 기준이 보입니다.</p>
<details><summary>${j(row.title, "은/는")} 매 경기 해야 하나요?</summary><p>아닙니다. 같은 문제가 두 경기 이상 반복될 때만 적용하면 충분합니다.</p></details>
<details><summary>${j(extended, "과/와")} ${support} 중 무엇이 먼저인가요?</summary><p>경기 중에는 ${j(extended, "이/가")} 먼저이고, 경기 후에는 ${j(support, "이/가")} 먼저입니다. 관찰과 기록의 순서를 나누세요.</p></details>
<details><summary>20분 연습은 어떻게 구성하나요?</summary><p>같은 조건을 10회 반복하고, 성공률보다 선택 기준이 흔들리지 않았는지 기록하는 방식이 좋습니다.</p></details>
<p>정리하면 ${j(row.title, "은/는")} 더 많은 정보를 외우는 글이 아니라, ${main}, ${extended}, ${j(support, "을/를")} 통해 다음 경기의 행동을 하나로 줄이는 글입니다. 오늘 한 장면을 고르고, 경기 중 신호를 확인하고, 경기 후 세 줄로 남기세요. 그 정도면 다음 경기에서 충분히 달라질 수 있습니다.</p>`;
}

const contentEntries = rows
  .map((row, index) => {
    const faq = [
      { question: `${j(row.tags[0], "은/는")} 무엇부터 봐야 하나요?`, answer: `${row.tags[1]} 신호를 먼저 보고 ${j(row.tags[2], "은/는")} 경기 후 기록으로 남기면 됩니다.` },
    ...
    highlight: ${esc(`${j(row.title, "은/는")} ${row.tags[0]}, ${row.tags[1]}, ${j(row.tags[2], "을/를")} 다음 경기에서 재확인할 기준으로 바꾸는 글입니다.`)},
